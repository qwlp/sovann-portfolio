...
find "$assets_dir/logos" -maxdepth 1 -type f -name '*.webp' -printf '%f\n' | sort -V | jq -Rsc 'split("\n") | map(select(length > 0))' > /tmp/sovann-logos.json
...
jq -n \
  ...
  --slurpfile logos /tmp/sovann-logos.json \
  --slurpfile poster /tmp/sovann-poster.json \
  --slurpfile photography /tmp/sovann-photography.json \
  '{design: $design[0], logos: $logos[0], poster: $poster[0], photography: $photography[0], video: []}' > "$assets_dir/gallery.json"
